...
//			 			}
				...
				cols.add("Insert ignore into `"+table+"` (");
				...
				if(!classtrfor.equals("")) {
					...
					if(classtrfor.contains("::")) {
						accountsmodel.dbactiondirectaction("INSERT INTO classteachers(school,Teacher,class,stream) "
								+ " values ((select "+ LoginController.getFarm()+",max(id) from teacherregistration),"
								+ " '"+classtrfor.split("::")[0]+"',"
								+ " '"+classtrfor.split("::")[1]+"')",2,0,0,0);	
								
					}else {
						accountsmodel.dbactiondirectaction("INSERT INTO classteachers(school,Teacher,class,stream) "
								+ " values ((select "+LoginController.getFarm()+",max(id) "
				...
				if(crunchifyListNew.toString().contains(":a:")) {								
					for(String s: crunchifyListNew){						
						accountsmodel.dbactiondirectaction("INSERT ignore INTO teaxchersubtought(school,tscnu,class,stream,sub) "
								+ " values ((select "+LoginController.getFarm()+",max(id) from "
				        ...
				        if(key.equals("classteacherfor")) {				        	
							...
							if(object.get("classteacherfor").equals("")) {								
								...
								if(classtrfor.contains("::")) {
									...
									accountsmodel.dbactiondirectaction("insert into  classteachers(school,teacher,class,stream)"
										+ " values("+LoginController.getFarm()+",'"+object.get("id")+"',"
										+ " '"+classtrfor.split("::")[0]+"',"
										+ " '"+classtrfor.split("::")[1]+"' )",2,0,0,0);
							...
							if(crunchifyListNew.toString().contains(":a:")) {														
								...
								for(String s: crunchifyListNew){
									...
									accountsmodel.dbactiondirectaction("INSERT INTO teaxchersubtought(school,tscnu,class,stream,sub) "
											+ " values ("+LoginController.getFarm()+",'"+object.get("id")+"','"+s.split(":a:")[0]+"',"
											+ " '"+(s.split(":a:")[1]).split(":b:")[0]+"','"+s.split("b:")[1]+"')",2,0,0,0);													
								}
